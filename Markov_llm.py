...
from Markov_chain import TrueMarkovChain
...
class MarkovLLM_GPT2_Expt(nn.Module):
    def __init__(self, cfg: ExptConfig):
        ...
        self.encoder = CausalEncoder(cfg.d_in, cfg.d_enc, layers=6, use_groupnorm=cfg.use_groupnorm)
        self.state   = StateUpdater(cfg.d_enc, cfg.d_state)
        # True Markov chain that works on encoded features for fair comparison
        self.true_markov = TrueMarkovChain(cfg.d_enc, cfg.d_state)
    ...
    def _prep_prompts(self, past: torch.Tensor, t_idx: torch.Tensor):
        ...
        if cfg.history_mode == "cur_last":
            s = self.state(e, t_idx)  # Use the original StateUpdater.forward() 
            prompts = self.proj_1(s)
        elif cfg.history_mode in ("true_markov_last", "true_markov_all"):
            # True Markov Chain on ENCODED features for fair comparison
            # P(S(t) | S(t-1), E(t)) = P(S(t) | S(1:t-1), E(1:t))
            seq_true_mc = self.true_markov(e)  # (B, L, d_state)
            
            # Add Markov loss to enforce meaningful state representations
            L_true_mc = self.true_markov.compute_markov_loss(e)
            losses["loss_true_markov"] = cfg.mc_weight * L_true_mc
            
            if cfg.history_mode == "true_markov_last":
                prompts = self.proj_1(seq_true_mc[:, -1, :])  # Use last Markov state
            else:  # true_markov_all
                n_seg = seq_true_mc.size(1) if (self.cfg.n_prompts <= 0) else min(self.cfg.n_prompts, seq_true_mc.size(1))
                pooled = segment_mean_pool(seq_true_mc, n_seg)
    ...
    def forward(self, past, t_idx, target_codes=None, target_patches=None, tie_weight=0.1, vq_weight: float = 1.0):
        ...
        if "loss_true_markov" in extra:
            losses["true_markov"] = extra["loss_true_markov"]
