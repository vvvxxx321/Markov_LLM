...
class MarkovChain(nn.Module):
    """Markov Chain where P(S(t) | S(t-1), X(t)) = P(S(t) | S(1:t-1), X(1:t))"""
    def __init__(self, d_in: int, d_state: int):
        ...
        # Markov transition: S(t) = f(S(t-1), X(t))
    ...
    def forward(self, past: torch.Tensor):
        """
        Build Markov states where each S(t) only depends on S(t-1) and X(t)
        Args:
